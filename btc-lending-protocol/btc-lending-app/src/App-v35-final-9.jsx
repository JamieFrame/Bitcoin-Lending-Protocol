  ...
  const checkNFTOwnership = async (loanId, userAddress, nftType) => {
    try {
      ...
      try {
        ...
        if (isOwner) {
          console.log(`✅ Loan ${loanId}: User owns ${nftType} NFT`);
        }
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '48px 16px' }}>
        ...
        <div style={{ background: '#1e293b', padding: '24px', borderRadius: '8px', marginBottom: '32px', overflowX: 'auto' }}>
          ...
          {marketplaceTab === 'myPositions' && (() => {
            ...
            // Check NFT ownership when tab first loads
            useEffect(() => {
              const loadMyPositions = async () => {
                // FIXED v35: Removed myPositionsNFT.length check
                if (!userAddress || loadingMyPositions) return;
                
                // FIXED v35: Only load when on myPositions tab
                if (marketplaceTab !== 'myPositions') return;
                
                setLoadingMyPositions(true);
                console.log('👥 Checking NFT ownership for all loans...');
                
                // FIXED v35: Check ALL loans, not just active
                const allLoans = openAuctions;
                const positions = [];
                ...
                console.log(`✅ Found ${positions.length} total positions (checked ${allLoans.length} loans)`);
              };
              
              loadMyPositions();
            }, [marketplaceTab, userAddress, openAuctions.length]);
