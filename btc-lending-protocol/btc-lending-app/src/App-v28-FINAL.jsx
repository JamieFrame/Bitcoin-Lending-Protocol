    ...
    try {
      ...
      await openContractCall({
        ...
        onFinish: () => {
          ...
          setTimeout(() => fetchOpenAuctions(), 5000);
        },
        onCancel: () => {
      ...
      if (error.message?.includes('u410')) {
        const auction = openAuctions.find(a => a.id === parseInt(loanId));
        const currentBidAmount = auction?.currentBid || auction?.maxRepayment || 0;
        showMessage(`❌ Bid must be LOWER than current bid (${currentBidAmount.toLocaleString()} USDT). This is a descending auction - each bid must be lower than the previous one!`, 'error');
      } else if (error.message?.includes('u408')) {
        showMessage('❌ Bid is too high! Must be at or below the maximum repayment amount.', 'error');
      } else if (error.message?.includes('u409')) {
        showMessage('❌ Bid is too low! Must cover at least the loan amount.', 'error');
      } else if (error.message?.includes('u406')) {
        showMessage('❌ Auction has ended! Cannot place bid on ended auction.', 'error');
      } else if (error.message?.includes('u1') || error.message?.includes('insufficient')) {
        showMessage('❌ Insufficient USDT balance. Click "Mint Test Tokens" above to get 500,000 USDT!', 'error');
      } else {
                      ...
                      if (!bidData || bidData === '(none)' || bidData === '0x09') {
                        ...
                        try {
                          ...
                          if (match) {
                            ...
                            console.log(`✅ Loan ${loanId} - Parsed bid (Method 1): ${currentBid} USDT`);
                            parsed = true;
                          }
                        } catch (e) {
                        ...
                        if (!parsed && bidData.includes('0x0a')) {
                          ...
                          if (match) {
                            const bidAmount = parseInt(match[1], 16);
                            currentBid = bidAmount / 1000000;
                            console.log(`✅ Loan ${loanId} - Parsed bid (Method 2): ${currentBid} USDT`);
                            parsed = true;
                          }
                        }
