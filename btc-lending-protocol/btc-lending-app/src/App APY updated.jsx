    ...
    return [...data].sort((a, b) => {
      ...
      if (sortKey === 'maxInterest') {
        ...
        if (aDays > 0) {
          ...
          aVal = (Math.pow(1 + aRate, 365 / aDays) - 1) * 100;
        } else {
          aVal = -1;
        }
        if (bDays > 0) {
          const bRate = (b.maxRepayment - b.borrowAmount) / b.borrowAmount;
          bVal = (Math.pow(1 + bRate, 365 / bDays) - 1) * 100;
        } else {
          bVal = -1;
        }
                ...
                <div>
                  ...
                  >
                    {(() => {
                      ...
                      const apy = ((Math.pow(1 + rate, 365 / days) - 1) * 100).toFixed(2);
                      return `${apy}%`;
                    })()}
                  </div>
                </div>
                ...
                <tbody>
                  {sortedLoans.map((loan) => {
                    ...
                    if (loanDays > 0) {
                      ...
                      loanAPY = ((Math.pow(1 + rate, 365 / loanDays) - 1) * 100).toFixed(2);
                    }
