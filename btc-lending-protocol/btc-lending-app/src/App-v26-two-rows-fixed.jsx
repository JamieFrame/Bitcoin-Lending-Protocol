...
function App() {
  ...
  const [maxRepayment, setMaxRepayment] = useState('60000');
  ...
  const transferCollateral = async () => {
    ...
    try {
      ...
      await openContractCall({
        ...
        onFinish: () => showMessage('Collateral transferred!', 'success')
      });
    } catch (error) {
      showMessage(`Error: ${error.message}`, 'error');
    } finally {
  ...
  const fetchOpenAuctions = async () => {
    ...
    try {
      ...
      const infoResponse = await fetch(`${CONFIG.NETWORK.coreApiUrl}/v2/info`);
      if (infoResponse.ok) {
      ...
      for (let i = 1; i <= totalLoans; i++) {
        ...
        console.log(`Loan ${i} parsed:`, { status, borrowAmount, maxRepayment, collateralAmount, auctionEndBlock, borrowerAddress });
      ...
      const infoResponse2 = await fetch(`${CONFIG.NETWORK.coreApiUrl}/v2/info`);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#e2e8f0' }}>
      ...
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '48px 16px' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ background: '#1e293b', padding: '24px', borderRadius: '8px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '16px', marginBottom: '16px' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '14px', color: '#94a3b8' }}>Collateral (BTC)</label>
                <input value={collateralAmount} onChange={(e) => setCollateralAmount(e.target.value)} placeholder="e.g., 1" style={{ width: '100%', padding: '12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#e2e8f0' }} />
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '14px', color: '#94a3b8' }}>Loan Amount (USDT)</label>
                <input value={requestedAmount} onChange={(e) => setRequestedAmount(e.target.value)} placeholder="e.g., 50000" style={{ width: '100%', padding: '12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#e2e8f0' }} />
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '14px', color: '#94a3b8' }}>Max Repayment (USDT)</label>
                <input value={maxRepayment} onChange={(e) => setMaxRepayment(e.target.value)} placeholder="e.g., 60000" style={{ width: '100%', padding: '12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#e2e8f0' }} />
                <div style={{ fontSize: '11px', color: '#64748b', marginTop: '4px' }}>Starting price</div>
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '14px', color: '#94a3b8' }}>Loan Duration (days)</label>
                <input value={duration} onChange={(e) => setDuration(e.target.value)} placeholder="e.g., 30" style={{ width: '100%', padding: '12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#e2e8f0' }} />
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '14px', color: '#94a3b8' }}>Auction Duration (days)</label>
                <input value={auctionDuration} onChange={(e) => setAuctionDuration(e.target.value)} placeholder="e.g., 1" style={{ width: '100%', padding: '12px', background: '#0f172a', border: '1px solid #334155', borderRadius: '6px', color: '#e2e8f0' }} />
              </div>
            </div>
            <div style={{ display: 'flex', gap: '16px' }}>
              <button onClick={transferCollateral} disabled={loading} style={{ flex: 1, padding: '12px', background: '#3b82f6', border: 'none', borderRadius: '6px', color: 'white', cursor: 'pointer', fontWeight: '600' }}>
                1. Transfer Collateral
              </button>
              <button onClick={createLoan} disabled={loading} style={{ flex: 1, padding: '12px', background: '#10b981', border: 'none', borderRadius: '6px', color: 'white', cursor: 'pointer', fontWeight: '600' }}>
                2. Create Loan
              </button>
            </div>
          </div>
        ...
        {/* Filter bar for auction/loan columns */}
        <div style={{ 
          background: '#1e293b', 
          padding: '12px 24px', 
          borderRadius: '8px',
        ...
        <div style={{ display: 'grid', gap: '32px', gridTemplateColumns: '1fr 1fr 1fr', marginBottom: '32px' }}>
          <div style={{ background: '#1e293b', padding: '24px', borderRadius: '8px' }}>
            <div style={{ marginBottom: '16px' }}>
              <h2 style={{ fontSize: '20px' }}>Active Auctions</h2>
            </div>

            {showMyLoansOnly && openAuctions.filter(auction => {
            ...
            {openAuctions.filter(a => {
              ...
              <div style={{ marginBottom: '24px', maxHeight: '300px', overflowY: 'auto' }}>
                <h3 style={{ fontSize: '16px', marginBottom: '12px', color: '#94a3b8' }}>
                  {showMyLoansOnly ? 'My Auctions' : 'Active Auctions'} ({(() => {
                    ...
                    if (!showMyLoansOnly) return activeAuctions.length;
                ...
                {openAuctions.filter(auction => {
                  ...
                  // Second filter: if My Loans Only is checked, filter by borrower
                  if (!showMyLoansOnly) return true;
          ...
          <div style={{ background: '#1e293b', padding: '24px', borderRadius: '8px' }}>
            ...
            {(() => {
              ...
              const endedAuctions = openAuctions.filter(auction => {
                ...
                // If My Loans Only is checked, filter by borrower
                if (showMyLoansOnly) {
                  const userAddress = userData?.profile?.stxAddress?.testnet;
                  if (!userAddress) return false;
                  return auction.borrowerAddress === userAddress;
