;; Bitcoin-USDT Lending Protocol v23
;; Descending auction system with tradeable NFT positions
;; FIX v21: Replaced USDT_CONTRACT and SBTC_CONTRACT constants with literal contract references
;; FIX v22: Changed all transfer-from to transfer when contract sends its own funds
;; FIX v23: Wrapped all contract transfers in as-contract to set tx-sender correctly
;; (Without as-contract, tx-sender remains the original caller, causing u101 errors)
  ...
  (let 
    ...
    (match current-bid
      prev-bid 
        (as-contract
          (try! (contract-call? .mock-usdt-v20 transfer
            (get amount prev-bid) tx-sender (get bidder prev-bid) none)))
      true)
    ...
    (match winning-bid
      bid-data
        (begin
          ...
          (as-contract
            (try! (contract-call? .mock-usdt-v20 transfer
              (get amount bid-data) tx-sender (get borrower loan) none)))
      ...
      (begin
        (as-contract
          (if (is-eq (get collateral-asset loan) ASSET_BTC)
            (try! (contract-call? .mock-sbtc-v20 transfer
              (get collateral-amount loan) tx-sender (get borrower loan) none))
            (try! (contract-call? .mock-usdt-v20 transfer
              (get collateral-amount loan) tx-sender (get borrower loan) none))))
...
(define-public (repay-loan (loan-id uint))
  (let 
    ...
    (as-contract
      (if (is-eq (get collateral-asset loan) ASSET_BTC)
        (try! (contract-call? .mock-sbtc-v20 transfer
          (get collateral-amount loan) tx-sender caller none))
        (try! (contract-call? .mock-usdt-v20 transfer
          (get collateral-amount loan) tx-sender caller none))))
