;; Bitcoin-USDT Lending Protocol v22
...
;; (Refunding bidders, paying borrowers, returning collateral - all use transfer now)
...
(define-public (place-bid (loan-id uint) (bid-amount uint))
  (let 
    ...
    (match current-bid
      prev-bid 
        (try! (contract-call? .mock-usdt-v20 transfer
          (get amount prev-bid) contract-addr (get bidder prev-bid) none))
      true)
    ...
    (match winning-bid
      bid-data
        (begin
          ...
          (try! (contract-call? .mock-usdt-v20 transfer
            (get amount bid-data) contract-addr (get borrower loan) none))
      ...
      (begin
        (if (is-eq (get collateral-asset loan) ASSET_BTC)
          (try! (contract-call? .mock-sbtc-v20 transfer
            (get collateral-amount loan) contract-addr (get borrower loan) none))
          (try! (contract-call? .mock-usdt-v20 transfer
            (get collateral-amount loan) contract-addr (get borrower loan) none)))
...
(define-public (repay-loan (loan-id uint))
  (let 
    ...
    (if (is-eq (get collateral-asset loan) ASSET_BTC)
      (try! (contract-call? .mock-sbtc-v20 transfer
        (get collateral-amount loan) contract-addr tx-sender none))
      (try! (contract-call? .mock-usdt-v20 transfer
        (get collateral-amount loan) contract-addr tx-sender none)))
